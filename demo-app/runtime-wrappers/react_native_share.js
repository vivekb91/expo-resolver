...
// Auto-generated runtime wrapper for react-native-share
const { createRuntimeResolver } = require('../runtime-resolver/src/index');

console.log('[Runtime Wrapper] Loading wrapper for react-native-share');
...
try {
  ...
  console.log('[Runtime Wrapper] Successfully wrapped react-native-share');
  
  // Handle default exports
  if (originalModule && originalModule.default) {
    wrappedModule.default = resolver.resolve('react-native-share', originalModule.default);
  }
  ...
  if (originalModule) {
    Object.keys(originalModule).forEach(key => {
      if (key !== 'default' && typeof originalModule[key] === 'object') {
        wrappedModule[key] = resolver.resolve('react-native-share.' + key, originalModule[key]);
      } else if (key !== 'default') {
  ...
  console.warn('[Runtime Wrapper] Failed to load react-native-share:', error.message);
